...
function renderGallery(items) {
  ...
  items.forEach((item, index) => {
    ...
    card.setAttribute('aria-label', `${item.title || 'Image'} from ${item.date}`);
    ...
    if (item.media_type === 'image') {
      ...
      link.innerHTML = `<a href="${item.url}" target="_blank" rel="noopener" aria-label="Open ${item.title} video in new tab">Open video in new tab</a>`;
  ...
  // Set focus to first gallery item for keyboard users
...
function openModal(item) {
  ...
  if (item.media_type === 'image') {
    ...
    iframe.setAttribute('title', `Video: ${item.title}`);
    iframe.setAttribute('aria-label', `Embedded video: ${item.title}`);
  ...
  explanation.setAttribute('aria-label', 'Image explanation');
  modalBody.appendChild(explanation);

  // Show modal
  modal.classList.remove('hidden');
  modal.setAttribute('aria-hidden', 'false');
  // Trap focus inside modal and set focus to close button
  modalClose.focus();
}
...
function closeModal() {
  ...
  modalBody.innerHTML = '';
  // Return focus to gallery when modal closes
